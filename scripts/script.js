...
let closeAlertBox = document.getElementsByClassName("close-alert-box");
let name_input = document.getElementsByClassName("user-name");
...
var check_level = window.location.href.split("?")[1].split("&")[1].split("=")[1].replace("+", " ");
var user_name = window.location.href.split("?")[1].split("&")[0].split("=")[1].replaceAll("+", " ");


if (check_level){
    console.log(check_level.length)
}
else{
    console.log(check_level.length)
}
...
for (let i = 0; i < level_indicator.length; i++) {
    ...
    name_input[0].firstElementChild.innerHTML = user_name
...
function alert_box(status, wrong, correct, life, math = 0) {
    document.getElementsByClassName("user-name")[1].innerHTML = user_name.toUpperCase()
...
// name_input[0].onclick=function(){
//     this
// }

closeAlertBox[0].onclick = function(){
    alert_box_parent[0].style.transform = "scale(0)";
